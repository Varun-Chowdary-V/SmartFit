...
class Reshaper:
  def __init__(self, label="female"):
    ...
    self.normals = np.load(open(os.path.join(utils.MODEL_DIR, "normals.npy"), "rb"), allow_pickle=True)
  ...
  def get_predict(self, flag, in_data):
    ...
    print("Contents of tmp:", tmp)
    if not np.issubdtype(tmp.dtype, np.number):
      raise ValueError("Input to IterativeImputer must be numeric.")
