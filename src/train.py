...
def obj2npy(label="SPRING"):
    print(' [**] begin obj2npy about %s... '%label)
    start = time.time()
    
    # Update the directory structure to match your data folder
    OBJ_DIR = os.path.abspath(os.path.join(DATA_DIR, "mesh"))
    obj_file_dir = OBJ_DIR  # All OBJ files are directly under 'mesh'
    print(f"Looking for OBJ files in {os.path.abspath(obj_file_dir)}...")

    try:
        file_list = sorted([f for f in os.listdir(obj_file_dir) if f.endswith(".obj")])
    except FileNotFoundError:
        print(f"Error: Directory {obj_file_dir} does not exist or is inaccessible.")
        return None

    if not file_list:
        print(f"Error: No OBJ files found in {obj_file_dir}.")
        return None
...
def convert_cp():
    ...
    for line in f:
        if '#' in line:
            if len(tmplist) != 0:
                # Normalize tmplist to ensure consistent sizes
                max_length = max(len(row) for row in tmplist)
                tmplist = [row + [0.0] * (max_length - len(row)) for row in tmplist]
                cp.append(tmplist)
                tmplist = []
    ...
    # Convert cp to a numpy array with dtype=object if still inhomogeneous
    try:
        cp_array = np.array(cp)
    except ValueError:
        cp_array = np.array(cp, dtype=object)

    # Save the processed data
    np.save(os.path.join(DATA_DIR, "cp.npy"), cp_array)

    print("cp shape:", np.shape(cp_array))
    print(' [**] finish convert_cp in %fs' % (time.time() - start))
    return cp_array
...
def rfe_multiprocess(i, dets, deform, body_num, x, measure, k_features):
  ...
  rfe = RFE(estimator=model, n_features_to_select=k_features, step=1, verbose=0)
  rfe.fit(x, y.ravel())
